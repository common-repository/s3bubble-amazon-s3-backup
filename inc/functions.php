...
function s3bubblebackup_exec($file, $action) {
	...
	fwrite($handle, "/**\n");
	fwrite($handle, " * SQL Dump created with Backup\n");
	fwrite($handle, " *\n");
	fwrite($handle, " */\n\n");
...
/*
 * Formats the database file size
 * @author sam
 * @params 
 */
function s3bubbleBackupFormatBytes($bytes, $precision = 2) { 
...
function database_size() {
    ...
    return '<li class="highlight-yes">Your database backup filesize will be <strong>' . s3bubbleBackupFormatBytes($dbsize) . '</strong> <i class="wp-menu-image dashicons-before dashicons-yes" style="color: green;"></i></li>';
...
function gzip_enabled() {
	...
	if(function_exists('gzopen')) {
        ...
        $results = '<li>GZIP is not available, AWS backup <strong>MAY</strong> not function correctly if you have it set <i class="wp-menu-image dashicons-before dashicons-no" style="color: red;"></i></li>';
    }
	return $results;
}
...
function exec_check($content_dir) {
	if(function_exists('exec')) {
		if(exec('echo EXEC') == 'EXEC'){
			 $output = shell_exec("cd {$content_dir} && du -sh --exclude='awsbackups*'");
...
function system_files() {
    ...
    $DirectoryCheck = WP_CONTENT_DIR . "/awsbackups";
	if(!empty($DirectoryCheck)) {
		if(!is_dir($DirectoryCheck) && !$is_safe_mode) {
			@mkdir($DirectoryCheck, 0777, true);
			@chmod($DirectoryCheck, 0777);
	
			if(is_dir($DirectoryCheck))
				$s3bubble_msg[] = 'Folder <strong>' . $DirectoryCheck . '</strong> was successfully created!';
			else
				$s3bubble_msg[] = $is_safe_mode ? 'PHP Safe Mode is active' : 'Folder <strong>' . $DirectoryCheck . '</strong> was not created. Check permissions!';								
		}
		else
			$s3bubble_msg[] = 'Folder <strong>' . $DirectoryCheck . '</strong> is available<i class="wp-menu-image dashicons-before dashicons-yes" style="color: green;"></i>';
		
		if(is_dir($DirectoryCheck)) {
			$dirloops = array('.htaccess', 'index.html');	
			foreach($dirloops as $dirloop) {
				if(!file_exists($DirectoryCheck . '/' . $dirloop)) {
					if($file = @fopen($DirectoryCheck . '/' . $dirloop, 'w'))  {	
						...
						$s3bubble_msg[] = 'File <strong>' . $dirloop . '</strong> was not created. Check permissions!';
				}
				else
